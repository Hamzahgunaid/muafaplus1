...
const RISK_BG: Record<string, string> = {
  LOW: '#E6F4EC', MODERATE: '#FDF3E1', HIGH: '#FDECE2', CRITICAL: '#FBE5E5',
}
const RISK_COLOR: Record<string, string> = {
  LOW: '#197540', MODERATE: '#B8771F', HIGH: '#D85A30', CRITICAL: '#D64545',
}
...
export default function DashboardPage() {
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="flex items-center justify-between mb-8">
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold mb-2"
                ...
                <div className="w-1.5 h-1.5 rounded-full" style={{ background: '#228C4E' }} />
                متصل بالنظام
              </div>
              ...
              <p className="text-sm mt-1" style={{ color: '#5A6478' }}>
                لوحة تحكم المدير — ملخص إحالات المنصة
              </p>
            </div>
            <button
          ...
          <div className="grid grid-cols-3 gap-4 mb-8">
            {[
              ...
              { label: 'قيد المعالجة',   value: referrals.length - completed, icon: '⏳', color: '#E87A2F', bg: '#FDF3E1' },
            ].map((stat, i) => (
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'white', border: '1px solid #EEF0F5' }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EEF0F5' }}>
              ...
              <button onClick={fetchReferrals} className="text-xs px-3 py-1.5 rounded-lg"
                ...
                <tbody>
                  {referrals.map(r => (
                    <tr key={r.referralId} className="hover:bg-gray-50 transition" style={{ borderTop: '1px solid #EEF0F5' }}>
                      ...
                      <td className="px-6 py-3">
                        {r.riskLevel ? (
                          <span className="px-2.5 py-1 rounded-full text-xs font-semibold"
                            style={{ background: RISK_BG[r.riskLevel] ?? '#EEF1F7', color: RISK_COLOR[r.riskLevel] ?? '#1E3A72' }}>
                      ...
                      <td className="px-6 py-3">
                        <Link href={`/referrals/${r.referralId}`}
            ...
            {referrals.length >= 20 && (
              <div className="px-6 py-4 flex justify-between" style={{ borderTop: '1px solid #EEF0F5' }}>
                <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                  className="text-sm disabled:opacity-40" style={{ color: '#5A6478' }}>السابق</button>
                <span className="text-xs" style={{ color: '#8A93A6' }}>صفحة {page}</span>
                <button onClick={() => setPage(p => p + 1)} className="text-sm" style={{ color: '#5A6478' }}>التالي</button>
              </div>
            )}
          </div>
        </div>
  ...
  const stats = {
    ...
    completed: sessions.filter((s: any) => s.status === 'Completed' || s.status === 'complete').length,
    articles:  sessions.reduce((sum: number, s: any) => sum + (s.articleCount ?? s.totalArticles ?? 0), 0),
    cost:      sessions.reduce((sum: number, s: any) => sum + (s.totalCost ?? 0), 0),
  }
          ...
          {[
            ...
            <div key={i} className="rounded-2xl p-5" style={{ background: 'white', border: '1px solid #EEF0F5' }}>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: '#8A93A6' }}>
                  {stat.label}
                </span>
              </div>
              <div className="text-3xl font-bold" style={{ color: stat.color }}>{stat.value}</div>
        ...
        <div className="grid gap-6" style={{ gridTemplateColumns: '1fr 340px' }}>
          ...
          <div className="rounded-2xl overflow-hidden" style={{ background: 'white', border: '1px solid #EEF0F5' }}>
            <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #EEF0F5' }}>
              ...
              <button onClick={fetchSessions} className="text-xs px-3 py-1.5 rounded-lg"
                style={{ background: '#F6F7FB', color: '#5A6478', border: '1px solid #EEF0F5', fontFamily: 'IBM Plex Sans Arabic, system-ui' }}>
                تحديث
              </button>
            </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {sessions.map((s: any, i: number) => (
                    <tr key={s.sessionId ?? i} className="hover:bg-gray-50 transition" style={{ borderTop: '1px solid #EEF0F5' }}>
                      ...
                      <td className="px-4 py-3">
                        {s.riskLevel ? (
                          <span className="px-2.5 py-1 rounded-full text-xs font-semibold"
                            style={{ background: RISK_BG[s.riskLevel] ?? '#EEF1F7', color: RISK_COLOR[s.riskLevel] ?? '#1E3A72' }}>
                            {RISK_LABELS[s.riskLevel as RiskLevel] ?? s.riskLevel}
                          </span>
                        ) : '—'}
                      </td>
                      <td className="px-4 py-3 text-sm text-center" style={{ color: '#0E1726' }}>
                        {s.articleCount ?? s.totalArticles ?? 0}
                      </td>
                      <td className="px-4 py-3">
                        <span className="px-2.5 py-1 rounded-full text-xs font-semibold"
                          style={{
                            background: (s.status === 'Completed' || s.status === 'complete') ? '#E6F4EC' : '#EEF1F7',
                            color:      (s.status === 'Completed' || s.status === 'complete') ? '#197540' : '#1E3A72',
                          }}>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="rounded-2xl p-6 text-white"
              style={{ background: 'linear-gradient(135deg, #1E3A72, #11254A)' }}>
              <div className="flex items-center gap-2 mb-4">
                <div className="w-2 h-2 rounded-full animate-pulse" style={{ background: '#F39A4F' }} />
