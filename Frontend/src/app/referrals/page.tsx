    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-6 py-8">
        ...
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-xl font-bold text-gray-900">الإحالات</h1>
          <Link
            href="/referrals/new"
        ...
        <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            <h2 className="font-semibold text-gray-800">قائمة الإحالات</h2>
            <button onClick={fetchReferrals} className="text-brand-600 text-sm hover:underline">
              تحديث
            </button>
          </div>
...
function ReferralCard({ referral: r }: { referral: ReferralResponse }) {
  // Engagement dots derived from referral status field (engagement detail requires separate call)
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {r.riskLevel && RISK_CLASS[r.riskLevel] && (
              <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-medium ${RISK_CLASS[r.riskLevel]}`}>
